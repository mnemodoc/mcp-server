module MnemodocServer
  module Tools
    ...
    class Status
      ...
      # the indexed vector width, active search mode, and the SQLite database
      # path. Takes no arguments; ignores the progress reporter.
      #
      # `embedding_dim` describes the INDEX, where `model` describes the
      # configuration — the two can disagree, and that disagreement is exactly
      # what stops semantic search. It is null while nothing has been embedded,
      # which is a state and not a fault: the key is always present, so a reader
      # can tell "no vectors yet" from an older payload that never carried it.
      def call(args : Hash(String, JSON::Any), progress : MCP::Progress? = nil) : MCP::ToolResult
        structured = {
          ...
          "embedding_dim" => @store.embedding_dim.try { |dim| JSON::Any.new(dim.to_i64) } || JSON::Any.new(nil),
